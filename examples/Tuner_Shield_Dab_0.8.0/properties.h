...
{
  //Interrupts
  //0x0000 INT_CTL_ENABLE Interrupt enable property; Default 0x0000
  INT_CTL_ENABLE = 0x0000,
  //0x0001 INT_CTL_REPEAT Interrupt repeat property; Default 0x0000
  INT_CTL_REPEAT = 0x0001,
  //0x0800 PIN_CONFIG_ENABLE Pin configuration property 0x8001; INTBOUTEN[15], I2SOUTEN[1], DACOUTEN[0]; Default 0x0001
  PIN_CONFIG_ENABLE = 0x0800,

  //Digital Audio
  //0x0200 DIGITAL_IO_OUTPUT_SELECT Selects digital audio Master or Slave; Default 0
  DIGITAL_IO_OUTPUT_SELECT = 0x0200,
  //0x0201 DIGITAL_IO_OUTPUT_SAMPLE_RATE Sets output sample audio rate in units of 1Hz; Default 48000
  DIGITAL_IO_OUTPUT_SAMPLE_RATE = 0x0201,
  //0x0202 DIGITAL_IO_OUTPUT_FORMAT Configure digital output format; Default 0x1800
  DIGITAL_IO_OUTPUT_FORMAT = 0x0202,
  //0x0203 DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_1 Deviations from the standard framing mode; Default 0
  DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_1 = 0x0203,
  //0x0204 DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_2 Deviations from the standard framing mode; Default 0
  DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_2 = 0x0204,
  //0x0205 DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_3 Deviations from the standard framing mode; Default 0
  DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_3 = 0x0205,
  //0x0206 DIGITAL_IO_OUTPUT_FORMAT_OVERRIDES_4 Deviations from the standard framing mode; Default 0
...
enum PROPERTIES_DAB
{
  //0x1710 DAB_TUNE_FE_VARM DAB/DMB Front End Varactor configuration slope; default 0
  DAB_TUNE_FE_VARM = 0x1710,
  //0x1711 DAB_TUNE_FE_VARB DAB/DMB Front End Varactor configuration intercept; default 0
  DAB_TUNE_FE_VARB = 0x1711,
  //0x1712 DAB_TUNE_FE_CFG Configure VHFSW switch from pin to ground; default 0x0001
  ...
  DAB_ACF_SOFTMUTE_ATTACK_Time = 0xB506,
...
enum PROPERTIES_FM
{
  ...
  FM_TUNE_FE_VARM            = 0x1710,//0x1710 FM_TUNE_FE_VARM FM Front End Varactor configuration slope. 0x0000
  FM_TUNE_FE_VARB            = 0x1711,//0x1711 FM_TUNE_FE_VARB FM Front End Varactor configuration intercept. 0x0000
  FM_TUNE_FE_CFG             = 0x1712,//0x1712 FM_TUNE_FE_CFG Additional configuration options for the front end. 0x0000
  
  //Band Config
  FM_SEEK_BAND_BOTTOM        = 0x3100,//0x3100 FM_SEEK_BAND_BOTTOM Sets the lower seek boundary of the FM band in multiples of 10kHz. 8750
  FM_SEEK_BAND_TOP           = 0x3101,//0x3101 FM_SEEK_BAND_TOP Sets the upper seek boundary for the FM band in multiples of 10kHz. 10790
  FM_SEEK_FREQUENCY_SPACING  = 0x3102,//0x3102 FM_SEEK_FREQUENCY_SPACING Sets the frequency spacing for the FM band in multiples of 10kHz when performing a seek. 10
  
  //Valid Tune
  FM_VALID_MAX_TUNE_ERROR    = 0x3200,//0x3200 FM_VALID_MAX_TUNE_ERROR Sets the maximum frequency error allowed before setting the AFCRL indicator. 114
  FM_VALID_RSSI_TIME         = 0x3201,//0x3201 FM_VALID_RSSI_TIME Sets the amount of time in ms to allow the RSSI/ISSI metrics to settle before evaluating. 15
  FM_VALID_RSSI_THRESHOLD    = 0x3202,//0x3202 FM_VALID_RSSI_THRESHOLD Sets the RSSI threshold for a valid FM Seek/Tune. 17
  FM_VALID_SNR_TIME          = 0x3203,//0x3203 FM_VALID_SNR_TIME Sets the amount of time in ms to allow the SNR metric to settle before evaluating. 40
  FM_VALID_SNR_THRESHOLD     = 0x3204,//0x3204 FM_VALID_SNR_THRESHOLD Sets the SNR threshold for a valid FM Seek/Tune. 10
  FM_VALID_HDLEVEL_THRESHOLD = 0x3206,//0x3206 FM_VALID_HDLEVEL_THRESHOLD Sets the HDLEVEL threshold for a valid FM Seek/Tune. 0

  //Receive Signal Quality
  FM_RSQ_INTERRUPT_SOURCE = 0x3300,//0x3300 FM_RSQ_INTERRUPT_SOURCE Configures interrupt related to Received Signal Quality metrics. 0
  FM_RSQ_SNR_HIGH_THRESHOLD = 0x3301,//0x3301 FM_RSQ_SNR_HIGH_THRESHOLD sets the high threshold, which triggers the RSQ interrupt if the SNR is above this threshold. 127
  FM_RSQ_SNR_LOW_THRESHOLD = 0x3302,//0x3302 FM_RSQ_SNR_LOW_THRESHOLD sets the low threshold, which triggers the RSQ interrupt if the SNR is below this threshold. -128
  FM_RSQ_RSSI_HIGH_THRESHOLD = 0x3303,//0x3303 FM_RSQ_RSSI_HIGH_THRESHOLD sets the high threshold, which triggers the RSQ interrupt if the RSSI is above this threshold. 127
  FM_RSQ_RSSI_LOW_THRESHOLD = 0x3304,//0x3304 FM_RSQ_RSSI_LOW_THRESHOLD Sets low threshold which triggers the RSQ interrupt if the RSSI is below this threshold. -128
  FM_RSQ_HD_DETECTION = 0x3307,//0x3307 FM_RSQ_HD_DETECTION Configures the Fast HD Detection routine.0x000d
  FM_RSQ_HD_LEVEL_TIME_CONST = 0x3308,//0x3308 FM_RSQ_HD_LEVEL_TIME_CONST Configures the Fast HD Detection Level Metric Filtering Time Constant. 32
  FM_RSQ_HDDETECTED_THD = 0x3309,//0x3309 FM_RSQ_HDDETECTED_THD Configures the HD Level Detected Threshold. 0x1e1e

  FM_ACF_INTERRUPT_SOURCE = 0x3400,        //0x3400 FM_ACF_INTERRUPT_SOURCE Enables the ACF interrupt sources. 0
  FM_ACF_SOFTMUTE_THRESHOLD = 0x3401,     //0x3401 FM_ACF_SOFTMUTE_THRESHOLD Sets the softmute interrupt threshold in dB attenuation. 31
  FM_ACF_HIGHCUT_THRESHOLD = 0x3402,      //0x3402 FM_ACF_HIGHCUT_THRESHOLD Sets the high cut interrupt threshold. 0
  FM_ACF_BLEND_THRESHOLD = 0x3403,       //0x3403 FM_ACF_BLEND_THRESHOLD Sets the stereo blend interrupt threshold.0
  FM_ACF_SOFTMUTE_TOLERANCE = 0x3404,    //0x3404 FM_ACF_SOFTMUTE_TOLERANCE Sets the distance from the final softmute value that triggers softmute convergence flag. 2
  FM_ACF_HIGHCUT_TOLERANCE = 0x3405,     //0x3405 FM_ACF_HIGHCUT_TOLERANCE Sets the distance from the final high cut frequency that triggers the high cut convergence flag. 20
  FM_ACF_BLEND_TOLERANCE = 0x3406,    //0x3406 FM_ACF_BLEND_TOLERANCE Sets the distance from the final blend state that triggers the blend convergence flag. 5

  FM_SOFTMUTE_SNR_LIMITS = 0x3500,     //Sets the SNR limits for soft mute attenuation. 0x0602
  FM_SOFTMUTE_SNR_ATTENUATION = 0x3501,  //Sets the SNR based softmute attenuation limits. 0x0008
  FM_SOFTMUTE_SNR_ATTACK_TIME = 0x3502,  //Sets the soft mute attack time. 16
  FM_SOFTMUTE_SNR_RELEASE_TIME = 0x3503,  //Sets the soft mute release time. 4000

  FM_AUDIO_DE_EMPHASIS = 0x3900,       //0x3900 FM_AUDIO_DE_EMPHASIS Sets the FM Receive de-emphasis. 0 = 75 ÃƒÅ½Ã‚Â¼s
  
  //RDS configuration
  FM_RDS_INTERRUPT_SOURCE = 0x3C00,  //0x3C00 FM_RDS_INTERRUPT_SOURCE This property configures interrupt related to RDS. 0x0000
  FM_RDS_INTERRUPT_FIFO_COUNT = 0x3C01, //0x3C01 FM_RDS_INTERRUPT_FIFO_COUNT Configures minimum received data groups in fifo before interrupt. 0x0000
  FM_RDS_CONFIG = 0x3C02,           //0x3C02 FM_RDS_CONFIG configures RDS settings to enable RDS processing (RDSEN) and set RDS block error thresholds. 0x0000
  FM_RDS_CONFIDENCE = 0x3C03//0x3C03 FM_RDS_CONFIDENCE Configures rds block confidence threshold. 0x1111
